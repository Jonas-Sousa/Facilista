...
const API = "https://json-facilista.vercel.app/romantico"
...
function MostraItens() {
  ...
  listaRomantica.forEach((obj, index) => {
    if (obj.checar) {
      ulItensComprados.innerHTML += `
...
function selecionarCheckbox() {
  ...
  checkboxes.forEach((checkbox) => {
    checkbox.addEventListener('click', (evento) => {
      ...
      listaRomantica[index].checar = evento.target.checked;
      MostraItens();
    });
  });
}
...
function deletarItens() {
  ...
  deletarObjetos.forEach((botao) => {
    botao.addEventListener('click', (evento) => {
      ...
      listaRomantica.splice(valorDoElemento, 1);
      MostraItens();
    });
  });
}
